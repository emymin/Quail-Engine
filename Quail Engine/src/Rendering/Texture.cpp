...
Texture::Texture(unsigned char* buffer,int width,int height,unsigned int internalformat,unsigned int format,unsigned int type):m_LocalBuffer(buffer),m_Width(width),m_Height(height),m_FilePath("INTERNAL"),m_BPP(0),m_RendererID(0)
{
	glGenTextures(1, &m_RendererID);
	glBindTexture(GL_TEXTURE_2D, m_RendererID);
